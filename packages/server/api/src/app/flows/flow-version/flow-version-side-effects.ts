...
async function handleSampleDataDeletion(projectId: ProjectId, flowVersion: FlowVersion, operation: FlowOperationRequest, log: FastifyBaseLogger): Promise<void> {
    ...
    switch (operation.type) {
        ...
        {
            const stepToDelete = flowStructureUtil.getStepOrThrow(operation.request.name, flowVersion.trigger, flowVersion.orphanSteps as import('@activepieces/shared').FlowAction[] | undefined)
        ...
        case FlowOperationType.DELETE_ACTION: {
            const stepsToDelete = operation.request.names.map(name => flowStructureUtil.getStepOrThrow(name, flowVersion.trigger, flowVersion.orphanSteps as import('@activepieces/shared').FlowAction[] | undefined))
